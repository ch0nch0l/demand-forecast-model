...
- Ensure you run notebooks from repo root, or adjust all relative paths.
- Create folders: `data/inputs`, `data/cleaned`, `data/training`, `outputs`, `reconcile_outputs`.

**2) Date parsing / monthly alignment**
- All downstream steps assume `Date` is monthly at **month-start**.
- If you see frequency errors, normalize dates to month-start before modeling.

**3) Large memory usage**
- `training_data_final.csv` can be large (hundreds of thousands of rows).
- In visualization, prefer loading only needed columns or use reconcile v2 outputs.

**4) TFT training slow**
- Reduce batch size
- Use fewer workers (macOS often uses `num_workers=0`)
- Use GPU if available

---

## 6) Suggested “Make it Production-Ready” Next Steps

If you plan to operationalize this pipeline:

1. Convert notebooks into scripts (CLI) with a single `config.yaml`
2. Add `requirements.txt` (or `poetry`/`uv`) with pinned versions
3. Add data validation (Great Expectations / pandera)
4. Add experiment tracking (MLflow)
5. Add CI checks: formatting (ruff/black), unit tests, smoke run on sample data

---

## 7) Contact / Ownership

- Data/Model Owner: _(fill in)_
- Engineering Owner: _(fill in)_
